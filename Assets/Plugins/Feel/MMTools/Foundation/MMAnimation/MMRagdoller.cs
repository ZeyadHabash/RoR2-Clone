...
namespace MoreMountains.Tools
{
    ...
    public class MMRagdoller : MonoBehaviour
    {
            ...
            set
            {
                ...
                else
                {
                    if (CurrentState == RagdollStates.Ragdolling)
                    {
                        ...
                        _ragdolledFeetPosition = 0.5f * (_animator.GetBoneTransform(HumanBodyBones.LeftToes).position +
        ...
        protected virtual void RegisterAnimatorParameters()
        {
            ...
            if (_animator.MMHasParameterOfType(_getUpFromBackAnimationParameterName,
                    AnimatorControllerParameterType.Bool))
            {
                _animatorParameters.Add(_getUpFromBackAnimationParameter);
            }

            if (_animator.MMHasParameterOfType(_getUpFromBellyAnimationParameterName,
                    AnimatorControllerParameterType.Bool))
            {
        ...
        protected virtual void HandleBlending()
        {
            if (CurrentState == RagdollStates.Blending)
            {
                if (Time.time <= _ragdollingEndTimestamp + _mecanimToGetUpTransitionTime)
                {
                    ...
                    Vector3 meanFeetPosition = 0.5f * (_animator.GetBoneTransform(HumanBodyBones.LeftFoot).position +
                                                       _animator.GetBoneTransform(HumanBodyBones.RightFoot).position);
                    Vector3 animatedDirection =
                        _animator.GetBoneTransform(HumanBodyBones.Head).position - meanFeetPosition;
                ...
                float ragdollBlendAmount = 1.0f -
                                           (Time.time - _ragdollingEndTimestamp - _mecanimToGetUpTransitionTime) /
                ...
                foreach (RagdollBodyPart bodypart in _bodyparts)
                {
                    if (bodypart.BodyPartTransform != transform)
                    {
                        ...
                        {
                            bodypart.BodyPartTransform.position = Vector3.Lerp(bodypart.BodyPartTransform.position,
                                bodypart.StoredPosition, ragdollBlendAmount);
                        }

                        bodypart.BodyPartTransform.rotation = Quaternion.Slerp(bodypart.BodyPartTransform.rotation,
                            bodypart.StoredRotation, ragdollBlendAmount);
                    }
                }
        ...
        public Vector3 GetPosition()
        {
            ...
            Vector3 newPosition = (_animator.GetBoneTransform(HumanBodyBones.Hips) == null)
        ...
        {
            Vector3 ragdollPosition = (_animator.GetBoneTransform(HumanBodyBones.Hips) == null)
                ? MainRigidbody.position
                : _animator.GetBoneTransform(HumanBodyBones.Hips).position;
