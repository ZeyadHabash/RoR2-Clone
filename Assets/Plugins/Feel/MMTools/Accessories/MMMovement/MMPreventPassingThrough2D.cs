    ...
    public class MMPreventPassingThrough2D : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            if (_lastMovementSquared > _squaredBoundsWidth)
            {
                ...
                {
                    _hitInfo = MMDebug.RayCast(_positionLastFrame, _lastMovement.normalized, movementMagnitude,
                ...
                if (_hitInfo.collider != null)
                {
                    ...
                    {
                        _hitInfo.collider.SendMessage("OnTriggerEnter2D", _collider,
                            SendMessageOptions.DontRequireReceiver);
                        if (RepositionRigidbodyIfHitTrigger)
                        {
                            this.transform.position = _hitInfo.point -
                                                      (_lastMovement / movementMagnitude) *
                    ...
                    {
                        Hit = _hitInfo;
                        this.gameObject.SendMessage("PreventedCollision2D", Hit,
                            SendMessageOptions.DontRequireReceiver);
