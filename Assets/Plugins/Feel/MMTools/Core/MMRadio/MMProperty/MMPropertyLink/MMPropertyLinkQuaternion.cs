...
namespace MoreMountains.Tools
{
    ...
    public class MMPropertyLinkQuaternion : MMPropertyLink
    {
        ...
        public override float GetLevel(MMPropertyEmitter emitter, MMProperty property)
        {
            ...
            float returnValue = MMMaths.Remap(axisValue, emitter.QuaternionRemapMinToZero,
        ...
        public override void SetLevel(MMPropertyReceiver receiver, MMProperty property, float level)
        {
            ...
            if (receiver.ModifyX)
            {
                float newX = MMMaths.Remap(level, 0f, 1f, receiver.QuaternionRemapZero.x,
                    receiver.QuaternionRemapOne.x);
                _newValue = _newValue * Quaternion.AngleAxis(newX, Vector3.right);
            }

            if (receiver.ModifyY)
            {
                float newY = MMMaths.Remap(level, 0f, 1f, receiver.QuaternionRemapZero.y,
                    receiver.QuaternionRemapOne.y);
                _newValue = _newValue * Quaternion.AngleAxis(newY, Vector3.up);
            }

            if (receiver.ModifyZ)
            {
                float newZ = MMMaths.Remap(level, 0f, 1f, receiver.QuaternionRemapZero.z,
                    receiver.QuaternionRemapOne.z);
