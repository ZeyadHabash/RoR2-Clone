...
namespace MoreMountains.Tools
{
    ...
    public class ShaderController : MMMonoBehaviour
    {
        ...
        [Tooltip("the Text with the shader you want to control")] [MMEnumCondition("TargetType", (int)TargetTypes.Text)]
        ...
        [Tooltip("the ramp along which to lerp when in ramp color mode")] [GradientUsage(true)]
        public Gradient ColorRamp;

        /// the color to lerp from	
        [Tooltip("the color to lerp from")] [ColorUsage(true, true)]
        public Color FromColor = Color.black;

        /// the color to lerp to	
        [Tooltip("the color to lerp to")] [ColorUsage(true, true)]
        ...
        [Tooltip("the duration of one loop")] public float LoopDuration = 1f;
        ...
        /// the noise frequency
        [Tooltip("the noise frequency")] [MMVector("Min", "Max")]
        public Vector2 Frequency = new Vector2(1f, 1f);

        /// the noise shift
        [Tooltip("the noise shift")] [MMVector("Min", "Max")]
        ...
        [Tooltip("the value to which to remap the random's zero bound")] [MMCondition("RemapNoiseValues", true)]
        public float RemapNoiseZero = 0f;

        /// the value to which to remap the random's one bound
        [Tooltip("the value to which to remap the random's one bound")] [MMCondition("RemapNoiseValues", true)]
        ...
        /// a test button for the one time shake
        [Tooltip("a test button for the one time shake")] [MMInspectorButton("ToDestination")]
        ...
        [Tooltip("the current value of the controlled float")] [MMReadOnly]
        public float CurrentValue;

        /// the current value of the controlled float, normalized
        [Tooltip("the current value of the controlled float, normalized")] [MMReadOnly]
        ...
        [Tooltip("the ID of the property")] [MMReadOnly]
        public int PropertyID;

        /// whether or not the property got found
        [Tooltip("whether or not the property got found")] [MMReadOnly]
        public bool PropertyFound = false;

        /// the target material
        [Tooltip("the target material")] [MMReadOnly]
        ...
        public virtual bool FindShaderProperty(string propertyName)
        {
            if (TargetType == TargetTypes.Renderer)
            {
                ...
                {
                    TargetRenderer.materials[TargetMaterialID] =
                        new Material(TargetRenderer.materials[TargetMaterialID]);
                }

                TargetMaterial = UseMaterialPropertyBlocks
                    ? TargetRenderer.sharedMaterials[TargetMaterialID]
                    : TargetRenderer.materials[TargetMaterialID];
            }
        ...
        protected virtual void UpdateValue()
        {
            ...
            switch (ControlMode)
            {
                ...
                case ControlModes.AudioAnalyzer:
                    CurrentValue = Mathf.Lerp(CurrentValue,
                        AudioAnalyzer.Beats[BeatID].CurrentValue * AudioAnalyzerMultiplier + AudioAnalyzerOffset,
                        AudioAnalyzerLerp * GetDeltaTime());
                ...
                case ControlModes.ToDestination:
                    ...
                    _remappedTimeSinceStart =
                        MMMaths.Remap(GetTime() - _startedTimestamp, 0f, ToDestinationDuration, 0f, 1f);
            ...
            if ((ControlMode == ControlModes.ToDestination) && _shaking &&
