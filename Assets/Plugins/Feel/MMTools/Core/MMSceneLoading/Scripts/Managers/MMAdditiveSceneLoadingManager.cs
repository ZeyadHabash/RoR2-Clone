    ...
    public class MMAdditiveSceneLoadingManager : MMMonoBehaviour
    {
        ...
        public static void LoadScene(string sceneToLoadName, MMAdditiveSceneLoadingManagerSettings settings)
        {
            LoadScene(sceneToLoadName, settings.LoadingSceneName, settings.ThreadPriority, settings.SecureLoad,
                ...
                settings.FadeMode, settings.UnloadMethod, settings.AntiSpillSceneName,
            ...
            List<MMSceneLoadingSpeedInterval> speedIntervals = null)
        {
            ...
            if (secureLoad)
            {
                ...
                {
                    Debug.LogError("MMLoadingSceneManagerAdditive : impossible to load the '" + sceneToLoadName +
                                   "' scene, " +
                ...
                {
                    Debug.LogError("MMLoadingSceneManagerAdditive : impossible to load the '" + loadingSceneName +
                                   "' scene, " +
                                   "there is no such scene in the project's build settings.");
                    return;
                }
        ...
        protected virtual void UpdateProgress()
        {
            ...
            if (_interpolateProgress)
            {
                _interpolatedLoadProgress = MMMaths.Approach(_interpolatedLoadProgress, _loadProgress,
                    Time.unscaledDeltaTime * ComputeInterpolationSpeed(_interpolatedLoadProgress));
        ...
        protected virtual IEnumerator LoadDestinationScene()
        {
            ...
            MMSceneLoadingManager.LoadingSceneEvent.Trigger(_sceneToLoadName,
                MMSceneLoadingManager.LoadingStatus.InterpolatedLoadProgressComplete);
