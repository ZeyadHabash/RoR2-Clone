...
namespace MoreMountains.Tools
{
    ...
    public class MMMultipleObjectPooler : MMObjectPooler
    {
        ...
        protected virtual GameObject GetPooledGameObjectOriginalOrder()
        {
            ...
            {
                GameObject findObject = FindInactiveObject(_objectPool.PooledGameObjects[_currentIndex].gameObject.name,
        ...
        protected virtual GameObject GetPooledGameObjectOriginalOrderSequential()
        {
            ...
            if (_objectPool.PooledGameObjects[_currentIndex].gameObject.activeInHierarchy)
            {
                GameObject findObject = FindInactiveObject(Pool[_currentIndex].GameObjectToPool.name,
                    _objectPool.PooledGameObjects);
                if (findObject != null)
                {
                    _currentCount++;
        ...
        protected virtual GameObject GetPooledGameObjectPoolSizeBased()
        {
            ...
            if (_lastPooledObject != null)
            {
                while (!CanPoolSameObjectTwice
                       && (_objectPool.PooledGameObjects[randomIndex].name == _lastPooledObject.name ||
                           !PoolObjectEnabled(_objectPool.PooledGameObjects[randomIndex]))
            ...
            if (_objectPool.PooledGameObjects[randomIndex].gameObject.activeInHierarchy)
            {
                ...
                GameObject pulledObject = FindInactiveObject(_objectPool.PooledGameObjects[randomIndex].gameObject.name,
                    _objectPool.PooledGameObjects);
                ...
                else
                {
                    ...
                    MMMultipleObjectPoolerObject searchedObject =
                        GetPoolObject(_objectPool.PooledGameObjects[randomIndex].gameObject);
        ...
        protected virtual GameObject GetPooledGameObjectRandomBetweenObjects()
        {
            ...
            {
                while (!CanPoolSameObjectTwice
                       && (Pool[randomIndex].GameObjectToPool.name == _lastPooledObject.name ||
                           !PoolObjectEnabled(Pool[randomIndex].GameObjectToPool))
                       && overflowCounter < _objectPool.PooledGameObjects.Count * OverflowAmount)
                {
                   ...
                   && overflowCounter < _objectPool.PooledGameObjects.Count)
            {
                ...
                GameObject newGameObject = FindInactiveObject(Pool[randomIndex].GameObjectToPool.name,
                    _objectPool.PooledGameObjects);
