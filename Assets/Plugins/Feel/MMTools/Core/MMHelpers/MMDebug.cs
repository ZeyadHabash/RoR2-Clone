    ...
    public static class MMDebug
    {
        ...
        public static void DebugLogCommand(string command)
        {
            ...
            if (splitCommand.Length > 1)
            {
                ...
                {
                    LogCommand(
                        "A version of command " + commandFirst +
                ...
                MMDebugLogCommandArgumentCountAttribute argumentAttribute = commandInfo
                    .GetCustomAttributes<MMDebugLogCommandArgumentCountAttribute>(true).FirstOrDefault();
                if (argumentAttribute != null && argumentAttribute.ArgumentCount > splitCommand.Length - 1)
                {
                    LogCommand(
                        "A version of command " + commandFirst + " needs at least " + argumentAttribute.ArgumentCount +
                        " arguments.", "#ff2a00");
                    return;
                }
            ...
            get
            {
                ...
                for (int i = 0; i < LogHistory.Count; i++)
                {
                    ...
                    log.Append("<color=#f9a682>[" +
                               MMTime.FloatToTimeString(LogHistory[i].Time, false, true, true, true) + "]</color> ");
        ...
        public static void DebugLogInfo(object message, string color = "", int timePrecision = 3,
        ...
        /// <param name="displayFrameCount"></param>
        /// <returns></returns>
        public static DebugLogItem LogDebugToConsole(object message, string color, int timePrecision,
            bool displayFrameCount)
        {
            DebugLogItem item = new DebugLogItem(message, color, Time.frameCount, Time.time, timePrecision,
                displayFrameCount);
            ...
            LayerMask mask, Color color, bool drawGizmo = false)
        {
            if (drawGizmo && DebugDrawEnabled)
            {
                ...
                points[2] = rotation *
                            (origin + (Vector2.right * halfSizeX) - (Vector2.up * halfSizeY)); // bottom right
        ...
        public static RaycastHit2D MonoRayCastNonAlloc(RaycastHit2D[] array, Vector2 rayOriginPoint,
            Vector2 rayDirection, float rayDistance, LayerMask mask, Color color, bool drawGizmo = false)
        {
            if (drawGizmo && DebugDrawEnabled)
            {
                Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color);
        ...
        /// <param name="color">Color.</param>
        /// <param name="drawGizmo">If set to <c>true</c> draw gizmo.</param>
        public static RaycastHit Raycast3D(Vector3 rayOriginPoint, Vector3 rayDirection, float rayDistance,
            LayerMask mask, Color color, bool drawGizmo = false,
        ...
        public static void InstantiateOnScreenConsole(int fontSize = 25)
        {
            ...
            if (_console == null)
            {
                ...
                GameObject loaded =
                    UnityEngine.Object.Instantiate(Resources.Load(_debugConsolePrefabPath) as GameObject);
        ...
        public static string GetSystemInfo()
        {
            ...
            result += "\n<color=#FFFFFF>Graphic Device Name :</color> " + SystemInfo.graphicsDeviceName + " (version " +
