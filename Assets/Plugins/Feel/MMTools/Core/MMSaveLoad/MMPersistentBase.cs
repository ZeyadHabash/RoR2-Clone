...
namespace MoreMountains.Tools
{
    ...
    public class MMPersistentBase : MonoBehaviour, IMMPersistent
    {
        ...
        [Tooltip("the object's unique ID")] [SerializeField] [MMReadOnly]
        protected string _guid;

        /// a debug button used to force a new GUI generation
        [MMInspectorButton("GenerateGuid")] public bool GenerateGuidButton;
        ...
        public virtual void ValidateGuid()
        {
            ...
            if (i == maxCount)
            {
                Debug.LogWarning(this.gameObject.name + " couldn't generate a unique GUID after " + maxCount +
                                 " tries, you should probably change its UniqueIDSuffix");
            }
        }
    }
}