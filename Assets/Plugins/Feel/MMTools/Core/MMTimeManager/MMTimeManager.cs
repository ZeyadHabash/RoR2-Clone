...
namespace MoreMountains.Feedbacks
{
    ...
    public struct MMTimeScaleEvent
    {
        ...
        public delegate void Delegate(MMTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp,
            ...
            MMTimeScaleLerpModes timeScaleLerpMode = MMTimeScaleLerpModes.Speed, MMTweenType timeScaleLerpCurve = null,
            float timeScaleLerpDuration = 0.2f,
            bool timeScaleLerpOnReset = false, MMTweenType timeScaleLerpCurveOnReset = null,
    ...
    public class MMTimeManager : MMSingleton<MMTimeManager>
    {
        ...
        [Tooltip("the time scale the system is lerping towards")] [MMReadOnly]
            ...
            if (_currentProperty.TimeScaleLerp)
            {
                if (_currentProperty.TimeScaleLerpMode == MMTimeScaleLerpModes.Speed)
                {
                    ...
                    ApplyTimeScale(Mathf.Lerp(Time.timeScale, TargetTimeScale,
                        Time.unscaledDeltaTime * _currentProperty.LerpSpeed));
                }
        ...
        protected virtual void SetTimeScale(TimeScaleProperties timeScaleProperties)
        {
            ...
            {
                timeScaleProperties.Duration =
                    Mathf.Max(timeScaleProperties.Duration, timeScaleProperties.TimeScaleLerpDuration);
                timeScaleProperties.Duration = Mathf.Max(timeScaleProperties.Duration,
                    timeScaleProperties.TimeScaleLerpDurationOnReset);
            }
        ...
        protected virtual void Unfreeze()
        {
            ...
            if (_timeScaleProperties.Count == 0)
            {
                if (_resetProperty.TimeScaleLerp && _resetProperty.TimeScaleLerpMode == MMTimeScaleLerpModes.Duration &&
                    _resetProperty.TimeScaleLerpOnReset)
                {
                    _lerpingBackToNormal = true;
                    MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, NormalTimeScale,
                        _resetProperty.TimeScaleLerpDuration, _resetProperty.TimeScaleLerp,
                        _resetProperty.LerpSpeed, true, MMTimeScaleLerpModes.Duration,
                        _resetProperty.TimeScaleLerpCurveOnReset, _resetProperty.TimeScaleLerpDurationOnReset);
                }
                else
                {
                    ResetTimeScale();
        ...
        public virtual void OnTimeScaleEvent(MMTimeScaleMethods timeScaleMethod, float timeScale, float duration,
