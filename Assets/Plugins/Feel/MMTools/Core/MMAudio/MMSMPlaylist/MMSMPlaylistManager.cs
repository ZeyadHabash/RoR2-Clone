...
namespace MoreMountains.Tools
{
    public class MMSMPlaylistManager : MMMonoBehaviour
    {
        ...
        [Tooltip("a global volume multiplier to apply when playing a song")] [Range(0f, 1f)]
        public float VolumeMultiplier = 1f;

        /// a pitch multiplier to apply to all songs when playing them
        [Tooltip("a pitch multiplier to apply to all songs when playing them")] [Range(0f, 20f)]
        ...
        [Tooltip("the index we're currently playing")] [MMReadOnly]
        ...
        [MMReadOnly] public MMStateMachine<PlaylistManagerStates> PlaylistManagerState;

        /// the time of the currently playing song
        [Tooltip("the time of the currently playing song")] [MMReadOnly]
        public float CurrentTime;

        /// the time (in seconds) left on the song currently playing 
        [Tooltip("the time (in seconds) left on the song currently playing")] [MMReadOnly]
        public float CurrentTimeLeft;

        /// the total duration of the song currently playing
        [Tooltip("the total duration of the song currently playing")] [MMReadOnly]
        public float CurrentClipDuration;

        /// the current normalized progress of the song currently playing
        [Tooltip("the current normalized progress of the song currently playing")] [Range(0f, 1f)]
        ...
        /// a next song test button
        [MMInspectorButton("QueueTargetSong")] public bool QueueTargetSongButton;
        ...
        [MMInspectorButton("ResetPlayCount")] public bool ResetPlayCountButton;

        /// a slider used to test volume control
        [Tooltip("a slider used to test volume control")] [Range(0f, 2f)]
        public float TestVolumeControl = 1f;

        /// a slider used to test speed control
        [Tooltip("a slider used to test speed control")] [Range(0f, 20f)]
        public float TestPlaybackSpeedControl = 1f;

        /// whether or not this playlist manager is currently playing
        public virtual bool IsPlaying =>
            (_currentlyPlayingAudioSource != null && _currentlyPlayingAudioSource.isPlaying);
        ...
        {
            if (
                ((Playlist.PlayOrder == MMSMPlaylist.PlayOrders.Random) ||
                 (Playlist.PlayOrder == MMSMPlaylist.PlayOrders.RandomUnique))
        ...
        public virtual void PlaySongAt(int songIndex)
        {
            ...
            _currentlyPlayingAudioSource =
                MMSoundManagerSoundPlayEvent.Trigger(Playlist.Songs[songIndex].Clip, _options);
        ...
        protected virtual void OnMMPlaylistVolumeMultiplierEvent(int channel, float newVolumeMultiplier,
            bool applyVolumeMultiplierInstantly = false)
        {
            ...
            {
                _currentlyPlayingAudioSource.volume =
                    Playlist.Songs[CurrentSongIndex].Options.Volume * VolumeMultiplier;
            }
        }

        protected virtual void OnMMPlaylistPitchMultiplierEvent(int channel, float newPitchMultiplier,
