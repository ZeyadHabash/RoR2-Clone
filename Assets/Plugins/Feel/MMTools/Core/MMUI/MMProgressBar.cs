...
namespace MoreMountains.Tools
{
    ...
    public class MMProgressBar : MMMonoBehaviour
    {
        ...
        [Tooltip("the main, foreground bar")] public Transform ForegroundBar;
        ...
        [MMCondition("SetInitialFillValueOnStart", true)] [Range(0f, 1f)] [Tooltip("the initial value of the bar")]
        ...
        [Tooltip("the speed at which to lerp the foreground bar")] [MMCondition("LerpForegroundBar", true)]
        ...
        [Tooltip("whether or not the delayed bar's animation should lerp")] [FormerlySerializedAs("LerpDelayedBar")]
        ...
        /// the speed at which to lerp the delayed bar
        [Tooltip("the speed at which to lerp the delayed bar")] [MMCondition("LerpIncreasingDelayedBar", true)]
        ...
        [Tooltip("the color to apply to the bar when bumping")] [MMCondition("ChangeColorWhenBumping", true)]
        public Color BumpColor = Color.white;

        /// the curve to map the bump animation on
        [Tooltip("the curve to map the bump animation on")] [FormerlySerializedAs("BumpAnimationCurve")]
        ...
        [Tooltip(
            "the curve to map the bump's intensity on. x is the normalized delta of the bump (from -1:-100% to 1:100%), y is the associated multiplier")]
        ...
        [Tooltip("the value towards which the bar is currently interpolating, ideally read only")] [Range(0f, 1f)]
        public float BarTarget;

        /// the current progress of the delayed bar increasing
        [Tooltip("the current progress of the delayed bar increasing")] [Range(0f, 1f)]
        public float DelayedBarIncreasingProgress;

        /// the current progress of the delayed bar decreasing
        [Tooltip("the current progress of the delayed bar decreasing")] [Range(0f, 1f)]
            ...
            if (_direction < 0)
            {
                ...
                if (_time - _lastUpdateTimestamp > DecreasingDelay)
                {
                    newFillDelayed = ComputeNewFill(LerpDecreasingDelayedBar, LerpDecreasingDelayedBarSpeed,
                        LerpDecreasingDelayedBarDuration, LerpDecreasingDelayedBarCurve, DecreasingDelay,
                ...
                else
                {
                    if (_time - _lastUpdateTimestamp > IncreasingDelay)
                    {
                        newFillDelayed = ComputeNewFill(LerpIncreasingDelayedBar, LerpForegroundBarSpeedIncreasing,
                            LerpForegroundBarDurationIncreasing, LerpForegroundBarCurveIncreasing, IncreasingDelay,
                            _delayedBarDecreasingProgress, out t2);
