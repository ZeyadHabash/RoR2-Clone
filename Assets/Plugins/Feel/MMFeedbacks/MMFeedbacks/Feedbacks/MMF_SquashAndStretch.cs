...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMF_SquashAndStretch : MMF_Feedback
    {
        ...
        {
            return (SquashAndStretchTarget == null);
        }

        public override string RequiredTargetText
        {
            get { return SquashAndStretchTarget != null ? SquashAndStretchTarget.name : ""; }
        }
        ...
        public override string RequiresSetupText
        {
            ...
            {
                return
                    "This feedback requires that a SquashAndStretchTarget be set to be able to work properly. You can set one below.";
            }
        }
#endif
        ...
        [Tooltip("the scale to reach when in ToDestination mode")] [MMFEnumCondition("Mode", (int)Modes.ToDestination)]
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            if (Active || Owner.AutoPlayOnEnable)
            {
                if ((Mode == Modes.Absolute) || (Mode == Modes.Additive))
                {
                    ...
                    _coroutine = Owner.StartCoroutine(AnimateScale(SquashAndStretchTarget, FeedbackDuration,
                        AnimateCurve, Axis, RemapCurveZero, RemapCurveOne * intensityMultiplier));
                }
        ...
        protected virtual IEnumerator ScaleToDestination()
        {
            ...
            while ((journey >= 0) && (journey <= FeedbackDuration) && (FeedbackDuration > 0))
            {
                ...
                float newScale = Mathf.LerpUnclamped(_initialAxisScale, DestinationScale,
                    AnimateCurve.Evaluate(percent) + Offset);
        ...
        /// <returns></returns>
        protected virtual IEnumerator AnimateScale(Transform targetTransform, float duration, AnimationCurve curve,
            PossibleAxis axis, float remapCurveZero = 0f, float remapCurveOne = 1f)
        {
        ...
        /// <param name="targetTransform"></param>
        protected virtual void ComputeAndApplyScale(float percent, AnimationCurve curve, float remapCurveZero,
            float remapCurveOne, Transform targetTransform)
        {
