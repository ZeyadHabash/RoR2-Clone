...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMF_Material : MMF_Feedback
    {
        ...
        public override string RequiresSetupText =>
            "This feedback requires that a TargetRenderer be set to be able to work properly. You can set one below.";
#endif
        ...
        {
            return (ComputedTimescaleMode == TimescaleModes.Scaled) ? Time.time : Time.unscaledTime;
        }

        public virtual float GetDeltaTime()
        {
            return (ComputedTimescaleMode == TimescaleModes.Scaled) ? Time.deltaTime : Time.unscaledDeltaTime;
        }
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            if (InterpolateTransition)
            {
                for (int i = 0; i < RendererMaterialIndexes.Length; i++)
                {
                    ...
                    _coroutines[i] = Owner.StartCoroutine(TransitionMaterial(
                        TargetRenderer.materials[RendererMaterialIndexes[i]], Materials[newIndex],
                        RendererMaterialIndexes[i]));
                }
            }
            else
            {
