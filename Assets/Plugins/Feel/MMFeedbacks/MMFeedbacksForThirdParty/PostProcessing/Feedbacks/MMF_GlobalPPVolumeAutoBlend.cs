...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMF_GlobalPPVolumeAutoBlend : MMF_Feedback
    {
        ...
        {
            return (TargetAutoBlend == null);
        }

        public override string RequiredTargetText
        {
            get { return TargetAutoBlend != null ? TargetAutoBlend.name : ""; }
        }
        ...
        public override string RequiresSetupText
        {
            ...
            {
                return
                    "This feedback requires that a TargetAutoBlend be set to be able to work properly. You can set one below.";
            }
        }
#endif
        ...
        /// the chosen mode
        [Tooltip("the chosen mode")] public Modes Mode = Modes.Default;

        /// the chosen action when in default mode
        [Tooltip("the chosen action when in default mode")] [MMFEnumCondition("Mode", (int)Modes.Default)]
        ...
        [Tooltip("the curve to apply to the blend")] [MMFEnumCondition("Mode", (int)Modes.Override)]
        public AnimationCurve BlendCurve = new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1f));

        /// the weight to blend from
        [Tooltip("the weight to blend from")] [MMFEnumCondition("Mode", (int)Modes.Override)]
        public float InitialWeight = 0f;

        /// the weight to blend to
        [Tooltip("the weight to blend to")] [MMFEnumCondition("Mode", (int)Modes.Override)]
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            if (TargetAutoBlend == null)
            {
                Debug.LogWarning(Owner.name +
                                 " : this MMFeedbackGlobalPPVolumeAutoBlend needs a TargetAutoBlend, please set one in its inspector.");
                return;
            }
            ...
            else
            {
                ...
                TargetAutoBlend.TimeScale = (ComputedTimescaleMode == TimescaleModes.Scaled)
                    ? MMGlobalPostProcessingVolumeAutoBlend.TimeScales.Scaled
                    : MMGlobalPostProcessingVolumeAutoBlend.TimeScales.Unscaled;
                if (!NormalPlayDirection)
                {
