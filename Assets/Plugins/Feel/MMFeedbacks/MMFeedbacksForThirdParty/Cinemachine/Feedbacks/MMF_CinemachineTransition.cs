...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMF_CinemachineTransition : MMF_Feedback
    {
		...
		public override float FeedbackDuration { get { return ApplyTimeMultiplier(BlendDefintion.m_Time); } set { BlendDefintion.m_Time
 = value; } }
#elif MM_CINEMACHINE3
        public override float FeedbackDuration
        {
            get { return ApplyTimeMultiplier(BlendDefintion.Time); }
            set { BlendDefintion.Time = value; }
        }
#endif
#if MM_CINEMACHINE || MM_CINEMACHINE3
...
#endif
#if MM_CINEMACHINE
		protected override void AutomateTargetAcquisition() => TargetVirtualCamera =
 FindAutomatedTarget<CinemachineVirtualCamera>();
#elif MM_CINEMACHINE3
        ...
        [Tooltip("the Cinemachine camera to target")] [MMFEnumCondition("Mode", (int)Modes.Binding)]
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            if (Mode == Modes.Event)
            {
                MMCinemachinePriorityEvent.Trigger(ChannelData, ForceMaxPriority, NewPriority, ForceTransition,
                    _tempBlend, ResetValuesAfterTransition, ComputedTimescaleMode);
            }
            else
            {
                MMCinemachinePriorityEvent.Trigger(ChannelData, ForceMaxPriority, 0, ForceTransition, _tempBlend,
                    ResetValuesAfterTransition, ComputedTimescaleMode);
...
#if MM_CINEMACHINE || MM_CINEMACHINE3
            MMCinemachinePriorityEvent.Trigger(ChannelData, ForceMaxPriority, 0, ForceTransition, _tempBlend,
                ResetValuesAfterTransition, ComputedTimescaleMode, true);
#endif
        }
    }
}