...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMF_Light2D_URP : MMF_Feedback
    {
        ...
        {
            get { return BoundLight != null ? BoundLight.name : ""; }
        }
        ...
        [Tooltip("the color to move to in destination mode")] [MMFEnumCondition("Mode", (int)Modes.ToDestination)]
        ...
        protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            switch (Mode)
            {
                ...
                case Modes.ShakerEvent:
                    ...
                    MMLight2DShakeEvent.Trigger(FeedbackDuration, RelativeValues, ModifyColor, ColorOverTime,
                        IntensityCurve,
                        RemapIntensityZero, RemapIntensityOne, FalloffCurve, RemapFalloffZero * intensityMultiplier,
                        RemapFalloffOne * intensityMultiplier,
        ...
        protected virtual void SetLightValues(float time, float intensityMultiplier)
        {
            ...
            {
                case Modes.OverTime:
                    intensity = MMFeedbacksHelpers.Remap(IntensityCurve.Evaluate(time), 0f, 1f, RemapIntensityZero,
                        RemapIntensityOne);
                    volumeIntensity = MMFeedbacksHelpers.Remap(VolumetricIntensityCurve.Evaluate(time), 0f, 1f,
                        RemapVolumetricIntensityZero, RemapVolumetricIntensityOne);
                    falloff = MMFeedbacksHelpers.Remap(FalloffCurve.Evaluate(time), 0f, 1f, RemapFalloffZero,
                        RemapFalloffOne);
                    shadowStrength = MMFeedbacksHelpers.Remap(ShadowStrengthCurve.Evaluate(time), 0f, 1f,
                ...
                case Modes.ToDestination:
                    ...
                    volumeIntensity = Mathf.Lerp(_initialVolumetricIntensity, ToDestinationVolumetricIntensity,
                        VolumetricIntensityCurve.Evaluate(time));
                    falloff = Mathf.Lerp(_initialFalloff, ToDestinationFalloff, FalloffCurve.Evaluate(time));
                    shadowStrength = Mathf.Lerp(_initialShadowStrength, ToDestinationShadowStrength,
                        ShadowStrengthCurve.Evaluate(time));
