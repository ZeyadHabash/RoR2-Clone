...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMF_FeedbackInspector
    {
        ...
        public virtual void OnEnable()
        {
            ...
            MMFHiddenPropertiesAttribute[] hiddenProperties = (MMFHiddenPropertiesAttribute[])_currentProperty.GetType()
        ...
        protected Dictionary<string, MMFConditionAttribute> _conditionDictionary =
            new Dictionary<string, MMFConditionAttribute>();

        protected Dictionary<string, MMFEnumConditionAttribute> _enumConditionDictionary =
            new Dictionary<string, MMFEnumConditionAttribute>();
            ...
            bool expandGroupInspectors)
        {
            ...
            for (int i = 0; i < fieldInfoLength; i++)
            {
                ...
                MMFInspectorGroupAttribute group =
        ...
        protected virtual void SearchForConditions(FieldInfo fieldInfo)
        {
            _conditionAttributeStore =
                Attribute.GetCustomAttribute(fieldInfo, typeof(MMFConditionAttribute)) as MMFConditionAttribute;
            ...
            _enumConditionAttributeStore =
                Attribute.GetCustomAttribute(fieldInfo, typeof(MMFEnumConditionAttribute)) as MMFEnumConditionAttribute;
                ...
                switch (currentProperty.type)
                {
                    ...
                    case _customTweenTypePropertyName:
                        ...
                        _mmTweenTypeProperty =
                            currentProperty.FindPropertyRelative(_findPropertyRelativeMMTweenDefinitionType);
                        if (_conditionDictionary.TryGetValue(currentProperty.name, out _conditionAttribute))
                        {
                            ...
                            string conditionPath = propertyPath.Replace(currentProperty.name,
                                _conditionAttribute.ConditionBoolean);
                            SerializedProperty sourcePropertyValue =
                                currentProperty.serializedObject.FindProperty(conditionPath);
                        ...
                        if (_enumConditionDictionary.TryGetValue(currentProperty.name, out _enumConditionAttribute))
                        {
                            string propertyPath = currentProperty.propertyPath;
                            string conditionPath = propertyPath.Replace(currentProperty.name,
                                _enumConditionAttribute.ConditionEnum);
                            SerializedProperty sourcePropertyValue =
                                currentProperty.serializedObject.FindProperty(conditionPath);
                        ...
                        EditorGUILayout.PropertyField(_mmTweenTypeProperty,
                            new GUIContent(currentProperty.displayName, currentProperty.tooltip));
                        if (_mmTweenTypeProperty.enumValueIndex == 0)
                        {
                            EditorGUILayout.PropertyField(
                                currentProperty.FindPropertyRelative(_mmTweenCurvePropertyName), _tweenCurveGUIContent);
                        }

                        if (_mmTweenTypeProperty.enumValueIndex == 1)
                        {
                            EditorGUILayout.PropertyField(currentProperty.FindPropertyRelative(_curvePropertyName),
